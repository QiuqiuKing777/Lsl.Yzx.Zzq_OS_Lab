...
## 六、知识点总结
基于这次实验的主题，我们将对调度算法设计过程中的一些主要因素及问题进行整理，并对主流的或已淘汰但~~风靡一时~~的调度算法进行整理如下：

### 调度算法研究的问题
对于大量的任务，有限的资源，设计某种规则来决定处理这些任务的顺序。这个任务是广义的，不仅限于操作系统中的进程调度，但此时我们以此为语境进行探讨。

在操作系统的调度体系中，主要有三个层次，分别是：
#### 高级调度（作业调度）
用户每提交一个作业，会让os启动一个程序，按一定的原则从外存的作业后备队列中选取一个调入内存，即为高级调度的任务。
一般来说，每个作业只被调度一次，调出后撤销。在我们的uCore中，其被体现为do_execve和do_exit.

#### 中级调度（内存调度）
当内存不够时，将某些进程从内存中暂时调出到外存，这样的进程进入挂起态suspend,链入挂起队列。这在uCore中无体现。

#### 低级调度（进程调度，又，处理机调度）
按某种策略，从就绪进程队列中选取一个合适的进程，为其分配处理机。在uCore中体现为schedule。在此次实验中，我们聚焦于低级调度。

### 低级调度的时机
一般来说，在中断处理过程中、操作系统内核程序临界区中以及原语中，应当禁用调度。

处理机的换出一般分为主动让渡和被动抢占两种，前者发生在exit、wait、运行中出现异常等情况下，后者发生在时间片耗尽、更高优先级进程抢占等情况。

### 调度算法的评价指标
 + cpu利用率：cpu忙碌时间/总时间

 + 系统吞吐量Throughput：完成作业数目/总时间
 ...
 + 等待时间：作业自从到达就绪队列上到等到低级调度的时间。

 + 响应时间：提交请求到首次产生响应的时间

 + 相应比response_ratio：（已等待时间+要求服务时间）/要求服务时间，越小性能越好。

### 一些调度算法
 + FCFS,First Come First Service：先来先做，是非抢占式算法，对短作业用户体验不好，不会产生饥饿。

 + SJF，Shortest Job First：短作业优先，非抢占（也有抢占版本，最短剩余时间有限），以作业的剩余执行时间为指标。存在饥饿情况。

 + HRRN，High Response Ratio Next：高相应比优先，以当前相应比作为调度的指标，这是对于SJF算法的改进，以免出现长作业饥饿现象。

 + RR,Round-Robin：时间片轮转，是uCore中使用的方法之一，胜在公平、简单，适用于分时操作系统。需要关注时间片设置的问题。
若时间片设置太小，会带来较大的进程调度开销，增大run-time的cpu占用，得不偿失；若时间片设计太大，会退化为FCFS算法，短作业相应比严重增加。

 + 优先级调度算法：对不同的proc设置合适的优先级，uCore中的stride算法即为一种优先级调度算法。一般来说，交互式的进程会被赋予更高的优先级，如用户输入的响应，而批处理式的进程，会被赋予较低的优先级，如科学计算进程等。会存在饥饿现象。

 + 多反馈队列调度：是一些调度算法的折衷。设置多级就绪队列，其优先级从高到低，每次调度分配的时间片从小到大（有类于不同存储器层次中越高层贵小快的特点）。
当且仅当更高层的就绪队列中为空时，才可为更底层的就绪队列分配时间片。所有新进入的进程都会进入最高级就绪队列。每用完一个时间片，将放入下一级就绪队列，若被中途抢占，放回到当前队列的队尾。显然，这也是一个会饥饿，抢占式的调度算法。

 + 多级队列调度。直接按照进程种类不同设计多个队列，不同队列间有优先级划分。习惯上，系统进程优先于交互式进程，优先于批处理进程。

### 多处理机调度
以上的讨论都基于单处理机的情况，而面对多处理机的情况，我们将多出一个问题（被调度的进程上哪个处理机？）和两大目标（负载均衡、处理机亲和性（缓存友好性））。

针对一个问题，我们使用公共就绪队列（天然负载均衡）或私有就绪队列（天然处理机亲和性）两种方式解决；

针对两大目标，我们使用软亲和或硬亲和的方式实现前者的处理机亲和性，使用推迁移或拉迁移的方式实现后者的负载均衡。
