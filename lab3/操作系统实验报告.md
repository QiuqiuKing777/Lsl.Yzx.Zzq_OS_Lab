...
​	实验3主要讲解的是中断处理机制。进行本次实验的目的主要是学习riscv的中断相关知识，掌握中断前后如何进行上下文环境的保存与恢复，最后实现对最简单断点中断和时钟中断的处理。
...
​	我们首先要在函数`include`处添加一个新的头文件`sbi.h`，然后在程序开头声明我们的静态变量`num`用于记录打印次数：
...
+ `sstatus`寄存器(Supervisor Status Register),它会保存当前状态
...
​	即首先调用`SAVE_ALL`汇编宏，用来保存所有寄存器到栈顶。在这个函数中，我们首先会拓展`36*REGBYTES`大小的空间，随后我们使用`STORE`指令将所有寄存器当前的值保存到内存中，对于前面说的几个特殊寄存器，我们需要先试用`csrr`指令将`CSR`读取到通用寄存器后，在进行保存。这样就形成了完整的`trapframe`。之后执行`move  a0, sp`操作，将栈顶的地址（即`trapframe`的起始地址）作为参数传入C语言处理函数中。最后通过`jal`指令，跳转并链接到`trap`函数中，进行处理。

##### C语言`trap`函数处理

​	`trap` 函数是中断 / 异常的逻辑处理核心，通过分析 `trapframe` 中的 `scause` 寄存器值，区分中断和异常并分发到对应处理函数。首先分析`scause`的值，若小于零，则表明最高位的值为1，说明是中断；否则说明是异常。
...
​	首先调用`RESTORE_ALL`宏，在这个宏中我们先加载两个`CSR`，随后恢复通用寄存器的值，将开辟的栈空间释放，然后将`sp`恢复到中断、异常发生前的位置。最后执行`sret`指令，CPU会根据`sepc`的值跳回中断、异常发生前的程序地址，恢复执行原程序。
...
​	`mov a0，sp`是连接汇编层保存上下文和C 语言层处理的关键操作，其核心目的是将 `trapframe`的地址传递给 C 语言函数。使得后续调用的`trap`函数能通过 `a0` 获取`trapframe`的指针，进而读取或修改上下文。
...
​	对于任何中断或异常，`__alltraps` 都必须保存所有通用寄存器。这样能够应对中断的异步性和函数调用约定，保证上下文完整性，避免代码执行错误。
...
​	`csrrw s0, sscratch, x0`：将 `sscratch` 的值读出到 `s0` 寄存器，同时将 `x0`（zero寄存器，值恒为 0）写入 `sscratch`。
